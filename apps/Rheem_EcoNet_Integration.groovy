...
def getDeviceDetails(id) {
    ...
    if (result != null) {
        ...
        result.results.locations.each { loc ->
            // Yup the API has a typo and called it "equiptments"
            deviceDetails.location_id = loc.location_id
            loc.equiptments.each { equip -> 
...
def createChildDevices() {
    for (waterHeater in waterHeaters)
    {
        ...
        device.updateDataValue("location_id", deviceDetails.location_id.toString())        
...
def setAwayMode(awayMode) {
    if (login()) {
        log.debug "Setting away mode to ${awayMode} via HTTP Post with location ${state.location_id}"
        def params = [ 
            uri: apiUrl,
            path: "/api/v/1/code/${systemKey}/setAway",
            timeout: 60,
            headers: [
                "ClearBlade-SystemKey": systemKey,
                "ClearBlade-SystemSecret": systemSecret,
            ...
            requestContentType: "application/json",
            body: ["location_id" : state.location_id, "@AWAY":awayMode]
        ] 
        ...
        log.debug "Set home mode API result: ${result}"
        return result
    }
    else
        return null    
